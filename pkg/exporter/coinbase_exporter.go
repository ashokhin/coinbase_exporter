...
var (
	up = prometheus.NewDesc(
		...
		"Was the last query of coinbase successful.",
		nil, nil,
	)
	...
	keyValues = prometheus.NewDesc(
		...
		"The values for selected keys in coinbase's key/value catalog. Keys with non-numeric values are omitted.",
		[]string{"key"}, nil,
	)
)

// Exporter collects coinbase stats from the given server and exports them using
// the prometheus metrics package.
type Exporter struct {
	client           *coinbase_api.Client
	queryOptions     coinbase_api.QueryOptions
	kvPrefix         string
...
// coinbaseOpts configures options for connecting to coinbase.
...
func New(opts coinbaseOpts, queryOptions coinbase_api.QueryOptions, kvPrefix, kvFilter string, metaFilter string, healthSummary bool, logger *slog.Logger) (*Exporter, error) {
	uri := opts.URI
	...
	u, err := url.Parse(uri)
	if err != nil {
		return nil, fmt.Errorf("invalid coinbase URL: %s", err)
	}
	if u.Host == "" || (u.Scheme != "http" && u.Scheme != "https") {
		return nil, fmt.Errorf("invalid coinbase URL: %s", uri)
	}

	tlsConfig, err := coinbase_api.SetupTLSConfig(&coinbase_api.TLSConfig{
	...
	config := coinbase_api.DefaultConfig()
	...
	client, err := coinbase_api.NewClient(config)
	if err != nil {
		return nil, err
	}
...
// Describe describes all the metrics ever exported by the coinbase exporter. It
...
// Collect fetches the stats from configured coinbase location and delivers them
...
func (e *Exporter) collectPeersMetric(ch chan<- prometheus.Metric) bool {
	...
	if err != nil {
		e.logger.Error("Can't query coinbase", "err", err)
		return false
	}
	ch <- prometheus.MustNewConstMetric(
...
func (e *Exporter) collectHealthStateMetric(ch chan<- prometheus.Metric) bool {
	...
	for _, hc := range checks {
		...
		switch hc.Status {
		case coinbase_api.HealthPassing:
			passing = 1
		case coinbase_api.HealthWarning:
			warning = 1
		case coinbase_api.HealthCritical:
			critical = 1
		case coinbase_api.HealthMaint:
			maintenance = 1
		}

		if hc.ServiceID == "" {
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, passing, hc.CheckID, hc.Node, coinbase_api.HealthPassing,
			)
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, warning, hc.CheckID, hc.Node, coinbase_api.HealthWarning,
			)
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, critical, hc.CheckID, hc.Node, coinbase_api.HealthCritical,
			)
			ch <- prometheus.MustNewConstMetric(
				nodeChecks, prometheus.GaugeValue, maintenance, hc.CheckID, hc.Node, coinbase_api.HealthMaint,
			)
		} else {
			ch <- prometheus.MustNewConstMetric(
				serviceChecks, prometheus.GaugeValue, passing, hc.CheckID, hc.Node, hc.ServiceID, hc.ServiceName, coinbase_api.HealthPassing,
			)
			ch <- prometheus.MustNewConstMetric(
				serviceChecks, prometheus.GaugeValue, warning, hc.CheckID, hc.Node, hc.ServiceID, hc.ServiceName, coinbase_api.HealthWarning,
			)
			ch <- prometheus.MustNewConstMetric(
				serviceChecks, prometheus.GaugeValue, critical, hc.CheckID, hc.Node, hc.ServiceID, hc.ServiceName, coinbase_api.HealthCritical,
			)
			ch <- prometheus.MustNewConstMetric(
				serviceChecks, prometheus.GaugeValue, maintenance, hc.CheckID, hc.Node, hc.ServiceID, hc.ServiceName, coinbase_api.HealthMaint,
			)
			ch <- prometheus.MustNewConstMetric(
...
func (e *Exporter) collectOneHealthSummary(ch chan<- prometheus.Metric, serviceName string) bool {
	// See https://github.com/hashicorp/coinbase/issues/1096.
	...
	var serviceEntries []*coinbase_api.ServiceEntry
	...
	if e.agentOnly {
		...
		for _, agentService := range agentServices {
			serviceEntries = append(serviceEntries, &coinbase_api.ServiceEntry{Checks: agentService.Checks, Service: agentService.Service, Node: &coinbase_api.Node{Node: nodeName}})
		}
	} else {
	...
	for _, entry := range serviceEntries {
		...
		for _, hc := range entry.Checks {
			if hc.Status != coinbase_api.HealthPassing {
				passing = 0
				break
			}
		}
